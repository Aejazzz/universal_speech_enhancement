...
This repo is the result of that design fully implemented: training pipeline, inference pipeline, evaluation pipeline, REST API, dashboard, and a 30-clip blind benchmark on the **Interspeech 2025 URGENT** validation set (mean ΔDNSMOS **+0.96**, 100 % top-rank, 0 Pareto-dominated picks — see [`outputs/reports/demo_blind_eval/report.md`](outputs/reports/demo_blind_eval/report.md)).
...
### 11.1 Blind eval — 30 clips from the URGENT validation set
...
| --- | ---: |
| Files processed | **30** |
| Mean ΔDNSMOS (OVRL) | **+0.96** |
| Mean ΔSIG | **+1.00** |
| Mean ΔBAK | **+1.69** |
| Mean ΔUTMOS22 | **−0.65** *(heuristic fallback; UTMOS not in routing blend)* |
